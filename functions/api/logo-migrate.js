...
if(ctx.total===0){body+='<div class="item ok"><b>✅ Migrasi Selesai!</b>Semua aplikasi sudah punya logo atau tidak ada yang perlu diproses. Sekarang Anda bisa menghapus file ini dari repo.</div>';body+='<a href="https://dash.cloudflare.com" class="btn red">Buka Dashboard Cloudflare (untuk hapus file)</a>'}
else if(ctx.results.length===ctx.batchSize){const next=ctx.offset+ctx.batchSize;const nextUrl=ctx.baseUrl+'?pass='+encodeURIComponent(ctx.pass)+'&batch='+ctx.batchSize+'&offset='+next;body+='<a href="'+nextUrl+'" class="btn">Lanjut ke batch berikutnya →</a>'}
else{body+='<div class="item ok"><b>✅ Batch Terakhir Selesai!</b>Klik tombol di atas untuk verifikasi bahwa semua aplikasi sudah diproses.</div>';body+='<a href="'+ctx.baseUrl+'?pass='+encodeURIComponent(ctx.pass)+'&batch='+ctx.batchSize+'&offset=0" class="btn">Jalankan Ulang dari Awal (untuk verifikasi)</a>'}
body+='<p class="muted" style="margin-top:16px;text-align:center">⚠️ Ingat: hapus file ini dari repo setelah migrasi selesai.</p>';
...
const targetUrl=findLogoUrl(name);
if(!targetUrl){ctx.results.push({name,status:'skip',msg:'Tidak ada di LOGO_MAP — abaikan atau isi manual lewat picker nanti.'});continue}
const slug=slugify(name);
if(!slug){ctx.results.push({name,status:'err',msg:'Nama tidak bisa dislugify.'});continue}
let u;
try{u=new URL(targetUrl)}catch(e){ctx.results.push({name,status:'err',msg:'URL tidak valid: '+targetUrl});continue}
const ctl=new AbortController();
const to=setTimeout(function(){ctl.abort()},8000);
let res;
try{res=await fetch(u.href,{redirect:'follow',signal:ctl.signal,headers:{'User-Agent':'CiccuLogoBot/1.0'}})}catch(e){clearTimeout(to);ctx.results.push({name,status:'err',msg:'Fetch gagal: '+(e&&e.message||e)});continue}
clearTimeout(to);
if(!res.ok){ctx.results.push({name,status:'err',msg:'HTTP '+res.status+' dari sumber'});continue}
const ct=(res.headers.get('content-type')||'').split(';')[0].trim().toLowerCase();
if(LOGO_CT.indexOf(ct)<0){ctx.results.push({name,status:'err',msg:'Bukan gambar ('+ct+')'});continue}
const buf=await res.arrayBuffer();
if(buf.byteLength>307200){ctx.results.push({name,status:'err',msg:'Terlalu besar ('+Math.round(buf.byteLength/1024)+' KB)'});continue}
try{await env.LOGOS.put('logos/'+slug,buf,{httpMetadata:{contentType:ct,cacheControl:'public, max-age=3600'}});await env.DB.prepare('UPDATE app_metadata SET logo_path=? WHERE app_name=?').bind(slug,name).run();ctx.results.push({name,status:'ok',msg:'Tersimpan: <b>'+slug+'</b>'});ctx.done++}catch(e){ctx.results.push({name,status:'err',msg:'Upload/DB gagal: '+(e&&e.message||e)})}
}
