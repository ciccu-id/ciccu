...
export async function getVariant(env,id){return env.DB.prepare('SELECT id,app_name,category,duration,price,status FROM pricelist WHERE id=?').bind(id).first()}
export async function listCatalog(env){const r=await env.DB.prepare("SELECT p.id,p.app_name,p.category,p.duration,p.price,p.status,p.sort_order,p.app_sort_order,COALESCE(rp.reseller_price,'') AS reseller_price,(SELECT COUNT(*) FROM rsl_stock_items s WHERE s.variant_id=p.id AND s.status='available') AS stock_available FROM pricelist p LEFT JOIN rsl_prices rp ON rp.variant_id=p.id ORDER BY COALESCE(p.app_sort_order,9999),COALESCE(p.sort_order,9999),p.id").all();return r.results}
export async function setResellerPrice(env,variantId,price){await env.DB.prepare('INSERT INTO rsl_prices(variant_id,reseller_price,updated_at) VALUES(?,?,?) ON CONFLICT(variant_id) DO UPDATE SET reseller_price=excluded.reseller_price,updated_at=excluded.updated_at').bind(variantId,price,nowStr()).run()}
...
export async function countAvailable(env,variantId){const r=await env.DB.prepare("SELECT COUNT(*) AS c FROM rsl_stock_items WHERE variant_id=? AND status='available'").bind(variantId).first();return r?r.c:0}
...
export async function addStock(env,variantId,fieldsObj){const r=await env.DB.prepare("INSERT INTO rsl_stock_items(variant_id,fields,status) VALUES(?,?, 'available')").bind(variantId,JSON.stringify(fieldsObj)).run();return r.meta.last_row_id}
export async function addStockBulk(env,variantId,arr){const stmts=arr.map(f=>env.DB.prepare("INSERT INTO rsl_stock_items(variant_id,fields,status) VALUES(?,?,'available')").bind(variantId,JSON.stringify(f)));if(!stmts.length)return 0;const res=await env.DB.batch(stmts);return res.length}
export async function updateStockFields(env,id,fieldsObj){await env.DB.prepare("UPDATE rsl_stock_items SET fields=? WHERE id=? AND status='available'").bind(JSON.stringify(fieldsObj),id).run()}
export async function disableStock(env,id){await env.DB.prepare("UPDATE rsl_stock_items SET status='disabled' WHERE id=? AND status='available'").bind(id).run()}
export async function deleteAvailableStock(env,id){await env.DB.prepare("DELETE FROM rsl_stock_items WHERE id=? AND status='available'").bind(id).run()}
...
try{const stmts=lines.map(l=>env.DB.prepare('INSERT INTO rsl_order_items(order_id,variant_id,app_name,category,duration,qty,unit_price,line_total) VALUES(?,?,?,?,?,?,?,?)').bind(orderId,l.variant_id,l.app_name,l.category,l.duration,l.qty,l.unit_price,l.qty*l.unit_price));if(stmts.length)await env.DB.batch(stmts);}catch(e){await env.DB.prepare('DELETE FROM rsl_orders WHERE id=?').bind(orderId).run();throw e}
return orderId;
}
...
const r=await env.DB.prepare("SELECT p.id,p.app_name,p.category,p.duration,COUNT(s.id) AS avail FROM pricelist p LEFT JOIN rsl_stock_items s ON s.variant_id=p.id AND s.status='available' LEFT JOIN rsl_prices rp ON rp.variant_id=p.id WHERE rp.variant_id IS NOT NULL GROUP BY p.id HAVING avail<=? ORDER BY avail ASC,p.app_name").bind(clampInt(threshold,5,1000)).all();
return r.results;
}
