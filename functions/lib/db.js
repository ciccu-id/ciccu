...
import{encryptJSON,decryptJSON}from'./crypto-creds.js';
function clampInt(v,d,m){const n=parseInt(v,10);if(isNaN(n))return d;return Math.max(0,Math.min(m,n))}
async function safeDecrypt(env,val){try{return JSON.stringify(await decryptJSON(env,val))}catch(e){return'{}'}}
...
for(const row of rows)row.fields=await safeDecrypt(env,row.fields);
return rows;
}
export async function addStock(env,variantId,fieldsObj,buyerNote){
const enc=await encryptJSON(env,fieldsObj||{});
const r=await env.DB.prepare("INSERT INTO rsl_stock_items(variant_id,fields,status,buyer_note) VALUES(?,?, 'available',?)").bind(variantId,enc,String(buyerNote||'').slice(0,500)).run();
return r.meta.last_row_id;
}
export async function addStockBulk(env,variantId,arr){
const encArr=await Promise.all((arr||[]).map(f=>encryptJSON(env,f||{})));
const stmts=encArr.map(e=>env.DB.prepare("INSERT INTO rsl_stock_items(variant_id,fields,status) VALUES(?,?,'available')").bind(variantId,e));
...
const enc=await encryptJSON(env,fieldsObj||{});
await env.DB.prepare("UPDATE rsl_stock_items SET fields=?,buyer_note=? WHERE id=? AND status IN ('available','disabled')").bind(enc,String(buyerNote||'').slice(0,500),id).run();
}
...
for(const rv of rev.results)rv.fields=await safeDecrypt(env,rv.fields);
...
const enc=await encryptJSON(env,fieldsObj||{});
const r=await env.DB.prepare('INSERT INTO rsl_order_revisions(order_item_id,fields,note,created_by,created_at) VALUES(?,?,?,?,?)').bind(orderItemId,enc,String(note||'').slice(0,1000),createdBy||'admin',nowStr()).run();
return r.meta.last_row_id;
}
