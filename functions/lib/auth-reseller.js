...
export function parseCookies(header){const out={};(header||'').split(';').forEach(p=>{const i=p.indexOf('=');if(i>0){const k=p.slice(0,i).trim();const v=p.slice(i+1).trim();try{out[k]=decodeURIComponent(v)}catch(e){out[k]=v}}});return out}
...
function rawSessionToken(request){const h=request.headers.get('x-reseller-token');if(h&&h.length>=32)return h.slice(0,128);const c=parseCookies(request.headers.get('cookie')||'')[SESSION_COOKIE];if(c&&c.length>=32)return c.slice(0,128);return null}
...
export async function getSession(env,request){const token=rawSessionToken(request);if(!token)return null;const th=await sha256hex(token);const row=await env.DB.prepare('SELECT s.id AS sid,s.expires_at,s.revoked_at,s.last_used_at,r.id,r.username,r.display_name,r.status FROM rsl_sessions s JOIN rsl_resellers r ON r.id=s.reseller_id WHERE s.token_hash=?').bind(th).first();if(!row||row.revoked_at||row.expires_at<=nowStr()||row.status!=='active')return null;const now=Date.now();const lu=row.last_used_at?Date.parse(String(row.last_used_at).replace(' ','T')+'Z'):0;if(!lu||now-lu>60000)await env.DB.prepare('UPDATE rsl_sessions SET last_used_at=? WHERE id=?').bind(nowStr(),row.sid).run();return{id:row.id,username:row.username,display_name:row.display_name,sid:row.sid}}
export async function revokeSession(env,request){const token=rawSessionToken(request);if(!token)return;const th=await sha256hex(token);await env.DB.prepare('UPDATE rsl_sessions SET revoked_at=? WHERE token_hash=? AND revoked_at IS NULL').bind(nowStr(),th).run()}
